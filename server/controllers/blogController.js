...
const getLatestBlogs = asyncHandler(async (req, res) => {
    //Go get all the blog posts from the database and store them in a variable called blogs."
    const blogs = await Blog.find()
    .populate('user')
    .populate('profile')
    .populate('category')
    //This sorts the blogs by their updatedAt date, with the most recent blogs appearing first
    .sort({ 'updatedAt': -1 })
    .limit(10);//WHY
...
const getBlogsByCategoryId = asyncHandler(async (req, res) => {
    //This searches for blog entries where the category field matches the categoryId provided in the URL parameters 
...
/* etrieves all blog entries written by a specific user (identified by userId from the URL), includes detailed information
 about the user, profile, and category for each blog entry, and then sends this data back to the client as a JSON response 
 with a 200 status code. */
const getBlog = asyncHandler(async (req, res) => {
    /* Blog.find({ user: req.params.userId }): Finds all blog entries where the user field matches the userId provided
...
const getBlogByBlogID = asyncHandler(async (req, res) => {
    /* 
 user: Fetches details about the blog's author.
profile: Fetches the author's profile information.
category: Fetches the details of the blog's category.
comments with profile: first get the comments and dor each comment Fetches details about the profiles of people who commented.
comments with user: first get the comments Fetches details about the users who wrote the comments. */
