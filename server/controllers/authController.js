...
const register = asyncHandler(async (req, res) => {
    ...
    // Hash Password
    // The salt ensures that even if two users have the same password, their hashed passwords will be different. 
    //This enhances security by protecting against attacks that exploit identical hashes
    const salt = await bcryptjs.genSalt(10);
    //This line mixes your password with the special ingredient (salt) to create a new, scrambled version of the password.
    // This scrambled version is what gets saved in the database.
    const hashedPassword = await bcryptjs.hash(password, salt);

    // Save user in database
    //Create and save user: Create a new User object with the hashed password and save it to the database.
    const user = new User({
        name,
        email,
...
const login = asyncHandler(async (req, res) => {
    ...
    //Verify password: Compare the provided password with the stored hashed password. If they don’t match,
...
const activateAccount = asyncHandler(async (req, res) => {
    ...
    try {
        ...
        // Set isActivated Property to tru
